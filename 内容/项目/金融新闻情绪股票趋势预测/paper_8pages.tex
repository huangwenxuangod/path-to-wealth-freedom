...
为直接检验“时间粒度是否决定结论”，本文进一步加入一个小规模公司级补充实验。数
据来自Hugging Face公开数据集\cite{luckycat}，筛选2019--2020年AAPL、MSFT、
GOOGL和TSLA四只高频提及股票，共得到1387篇文章、649个有新闻的公司--交易日，
再与Yahoo Finance日线行情对齐后形成2132个公司--交易日样本。该子样本至少包含
\texttt{date\_publish}和\texttt{mentioned\_companies}字段，能比DJIA基线更严格地
讨论训练期与测试期错位问题；但其中仍有15.9\%文章时间戳被标准化为午夜，说明它仍
不是理想的盘中事件流数据。
...
在公司级补充实验中，本文不再做多折年度切分，而是直接对最关键的问题进行验证：
一组采用静态划分，即2019年训练、2020年测试；另一组采用滚动重训，即使用过去6个
月训练、下一个自然月测试。所有模型均在公司--交易日面板上联合拟合，并比较仅量价
特征与量价加新闻特征。这样做的目的不是追求最优收益，而是观察当市场从2019年的
低波动环境进入2020年的疫情冲击阶段后，新闻特征和量价特征分别发生了什么漂移。
...
\subsection{公司级时间戳子样本验证}

图\ref{fig:timestampauc}给出了2019训练、2020测试的小样本公司级结果。Logistic
在静态划分下由0.517提高到0.530，XGBoost由0.507提高到0.524；若改为6个月滚动
重训，Logistic由0.501提高到0.519，XGBoost由0.508提高到0.514。也就是说，当数
据从“宏观标题--指数”换成“公司提及--个股”后，新闻增量仍然存在，但量级依旧不大，
而且滚动重训并没有自动把AUC推高到明显可交易的区间。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/timestamp_auc_static_vs_rolling.png}
\caption{公司级时间戳子样本中静态划分与滚动重训的AUC比较}
\label{fig:timestampauc}
\end{figure}

这个结果有两个含义。第一，用户担心的“2019训2020测太远”并不是空泛担忧；第二，
即便显式采用滚动更新，只要时间戳仍然粗糙、新闻量有限、标签仍是日级下一日方向，
性能改善也会很有限。滚动训练能缓解一部分概念漂移，但不能弥补标签窗口与信息半衰
期不匹配的问题。

进一步的漂移诊断见图\ref{fig:timestampdrift}。2019与2020之间，新闻侧的
\texttt{sentiment\_mean} PSI仅0.025，\texttt{article\_count} PSI仅0.010，说明
简单情绪均值和新闻数量分布并未剧烈改变；相反，价格侧\texttt{return\_1d}的PSI
为0.218，\texttt{volatility\_20d}的PSI高达0.534，表明真正强烈漂移的是市场状态。
这与疫情阶段波动率急升的常识一致，也说明为什么单纯依赖旧年份样本容易失真：模型
学到的不是“新闻如何影响价格”的稳定关系，而是“特定波动环境下新闻和价格偶然同向”
的局部模式。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/timestamp_feature_drift.png}
\caption{2019训练期与2020测试期的特征漂移：价格侧强于新闻侧}
\label{fig:timestampdrift}
\end{figure}
...
补充实验进一步表明，问题不只在文本模型。即便换到公司级数据，2019到2020之间
最明显的漂移也来自收益率和波动率，而不是日均情绪。这说明“旧年份太老”并不只是
样本数量问题，而是标签生成机制和市场状态已经发生变化。若真实alpha只在盘后公告、
财报意外或开盘前数小时内短暂存在，那么将它压缩成日级下一交易日方向，信号天然会
被稀释。
...
为验证时间粒度问题，本文还提供了一个独立的公司级脚本
\texttt{code/timestamp\_subset\_experiment.py}。该脚本会自动下载2019和2020年
的公开新闻子集、对四只股票抓取Yahoo Finance日线数据、聚合标题情绪、构造静态与
滚动测试，并输出\texttt{outputs/timestamp\_subset/}下的指标表、漂移表和两张图。
这一补充实验只使用子样本，因此不应被解释为最终交易系统，而应视为对“时间错位是否
会导致失真”这一方法论问题的快速验证。
...
比单纯提高模型复杂度更重要。进一步的公司级时间戳子样本验证显示：当训练期从
2019跨到2020时，新闻增量仍只带来约0.01--0.02量级的AUC改善，而价格侧波动率
漂移远强于新闻侧情绪漂移。换言之，真正需要解决的是regime shift、horizon定义和
发布时间精度，而不是继续在粗粒度日级标签上堆叠更复杂的分类器。
...
\bibitem{luckycat}
luckycat37. Financial News Dataset. Hugging Face Datasets, accessed on June 16, 2026.
