...
(async () => {
  ...
  for (let i = 0; i < total; i++) {
    await page.evaluate((idx) => {
      document.querySelectorAll('.slide').forEach((s, j) => s.classList.toggle('is-active', j === idx));
    }, i);
    await new Promise(r => setTimeout(r, 700));
    const padded = String(i + 1).padStart(2, '0');
    const out = `D:/path-to-wealth-freedom/.workbuddy/screenshots/v11-page${padded}.png`;
